...
// DATA SCHEMA
// ============================================
// Jobs will have this structure:
// {
//   id, title, company, location, region (Boston/NYC),
//   jobType (consulting/ib/internship),
//   postedDate, source, url, description,
...
async function fetchFromIndeed() {
  ...
  // Uses the free Indeed Publisher API
  try {
    // This is a placeholder - Indeed's API requires publisher ID
    // We'll implement actual scraping or use job aggregator instead
...
async function fetchFromJSearch() {
  // JSearch is a free job aggregator API
  // Returns jobs from multiple sources
  try {
...
function formatJSearchResults(jobs) {
  // Transform JSearch results to our schema
  return jobs.map(job => ({
    id: job.job_id,
    title: job.job_title,
...
app.get('/api/jobs', async (req, res) => {
  ...
  try {
    ...
    // Filter for relevant companies and locations
